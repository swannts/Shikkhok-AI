...
class ApiClient {
  ...
  private onSessionExpiredCallbacks: Set<SessionExpiredHandler> = new Set();
    ...
    try {
      ...
      if (raw && typeof raw === 'object') {
        ...
        if (errProp && typeof errProp === 'object') {
          ...
          errorCode = String((raw as Record<string, unknown>).errorCode || (raw as Record<string, unknown>).code || errorCode);
        }
      }
    } catch {
      // Body was not JSON
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    try {
      ...
      throw new NetworkError(
        err instanceof Error ? err.message : 'A network connection error occurred.',
      );
    }
  }
