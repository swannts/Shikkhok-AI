...
export const aiAdminService = {
  async getHealth(): Promise<AiHealthResponse> {
    return apiClient.get<AiHealthResponse>("/admin/ai/health");
  },

  async getStats(): Promise<IngestionStatsResponse> {
    return apiClient.get<IngestionStatsResponse>("/admin/ai/stats");
  },

  async ingestChunk(
    payload: IngestTextbookPayload,
  ): Promise<IngestChunkResponse> {
    return apiClient.post<IngestChunkResponse>("/admin/ai/ingest", payload);
  },

  async deleteBook(bookId: string): Promise<DeleteBookResponse> {
    return apiClient.delete<DeleteBookResponse>(
      `/admin/ai/books/${encodeURIComponent(bookId)}`,
    );
  },

  async testRetrieval(payload: RetrievalTestPayload) {
    return apiClient.post("/admin/ai/retrieval-test", payload);
  },
};
