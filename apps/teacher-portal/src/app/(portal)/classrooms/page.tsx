...
export default function ClassroomsPage() {
  ...
  useEffect(() => {
    teacherClassroomService
      .listSubjects(classLevel, "bangla", academicYear)
      .then((data) => {
        setSubjects(data);
        setSubjectId((current) =>
          data.some((subject) => subject._id === current)
            ? current
            : data[0]?._id || "",
        );
      })
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      await teacherClassroomService.createClassroom({
        ...
        section,
        medium: "bangla",
        curriculumYear: Number(academicYear),
      });
      setShowModal(false);
      setName("");
      fetchClassrooms();
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-slate-900">
            আমার শ্রেণিকক্ষসমূহ (Classrooms)
          </h1>
          <p className="text-xs text-slate-500 mt-0.5">
            শিক্ষার্থীদের ক্লাসে যুক্ত করতে প্রতিটি ক্লাসের স্বয়ংক্রিয় জয়েন কোড
            শেয়ার করুন
          </p>
        </div>
        <button
