...
| **Home Dashboard** | `GET /progress/me/summary`, `GET /gamification/me/summary` | `CurriculumRepositoryImpl` | `homeDashboardProvider` | `HomePage` | Real-time summary fetch with safe zero-state handling; type-safe `AppRoutes` navigation | `curriculum_repository_impl_test.dart` | **100% Complete** |
| **Curriculum Navigation** | `GET /curriculum/subjects`, `/chapters`, `/lessons` | `CurriculumRepositoryImpl` | `CurriculumController` | `LearnPage`, `SubjectDetailsPage`, `ChapterDetailsPage` | Hierarchical class & curriculum filtering; real chapter progress calculation (`${completed}/${total}` & completion rate) | `curriculum_repository_impl_test.dart`, `chapter_progress_test.dart`, `learning_flow_real_data_test.dart` | **100% Complete** |
| **Lesson Reader & Bookmarks** | `PUT /progress/me/lessons/:lessonId`, `POST /sync/me/batches` | `CurriculumRepositoryImpl`, `BookmarkRepositoryImpl` | `CurriculumController`, `LessonBookmarkNotifier` | `LessonReaderPage` | Device-local persistent & user-isolated secure storage for bookmarks; immediate UI state update + Drift SQLite persistent sync queue for background batch sync | `bookmark_persistence_test.dart`, `curriculum_repository_impl_test.dart`, `sync_local_data_source_test.dart` | **100% Complete** |
...
| **AI Tutor & Streaming Chat** | `GET/POST /tutor/me/conversations`, `POST /messages/stream` (SSE) | `TutorRepositoryImpl` | `TutorController` | `AiTutorChatPage`, `AiTutorHistoryPage` | Contextual lesson-scoped conversation launch (`lessonId`, `chapterId`, `subjectId`, `title`); Server-Sent Events (SSE) streaming with chunk-boundary buffering, typed deltas, textbook citation preservation, and cancelation | `tutor_controller_test.dart`, `tutor_mapper_test.dart`, `sse_parser_test.dart`, `lesson_scoped_tutor_test.dart` | **100% Complete** |
...
- **Flutter Automated Tests**: `flutter test` $\rightarrow$ **136 tests passed (100%)**.
...
- **Mobile Verification Script**: `verify_mobile.sh` $\rightarrow$ **100% Passed**.
...
5. **Lesson Bookmarks**: Tap bookmark in `LessonReaderPage` $\rightarrow$ State persisted to encrypted local storage under current user namespace `shikkhok_bookmarks_${userId}` $\rightarrow$ Survives application restart $\rightarrow$ Isolated from other user accounts.
6. **Lesson-Scoped AI Tutor**: Tap "AI শিক্ষকের সাহায্য প্রয়োজন?" in `LessonReaderPage` $\rightarrow$ Initiates backend conversation with real `lessonId`, `chapterId`, `subjectId`, and lesson title $\rightarrow$ Navigates to `/ai-tutor-chat/:conversationId` with active context preloaded.
7. **Chapter Progress Computation**: Return to `ChapterDetailsPage` $\rightarrow$ Real progress indicator displays live completion status (e.g. `1/4 পাঠ সম্পন্ন (25%)`) sourced directly from `/progress/me/subjects/:subjectId`.
8. **Practice & Instant Evaluation**: Start 5-question MCQ session $\rightarrow$ Select options $\rightarrow$ Server-side grading evaluates submission and returns step explanations.
9. **Exams & Assessments**: Start exam $\rightarrow$ Save answer sends `submittedAnswer` and flags send `flagged` $\rightarrow$ Countdown timer runs $\rightarrow$ Submit exam graded by server.
10. **AI Tutor Streaming & Citations**: Open AI Tutor $\rightarrow$ Ask "বীজগণিতীয় সূত্রাবলি বুঝিয়ে দাও" $\rightarrow$ SSE frames stream deltas in real-time, citations from NCTB textbook render with page numbers $\rightarrow$ Click Stop Generation $\rightarrow$ Stream cleanly closes without fake fallback text insertion.
11. **Offline Simulation**: Turn off network $\rightarrow$ Complete lesson / mark notification read $\rightarrow$ Operation is saved into Drift SQLite table `sync_queue` $\rightarrow$ Close app $\rightarrow$ Reopen app $\rightarrow$ Queue is intact $\rightarrow$ Reconnect network $\rightarrow$ Batch flush succeeds with `applied` status.
12. **Auth Expiration Simulation**: When 401 refresh token expires $\rightarrow$ `SessionManager` notifies app $\rightarrow$ `AuthController` transitions to `Unauthenticated` $\rightarrow$ User is redirected to `/login` without crash.
